export const SW_AI_MOBILE_LOCALE = {
  ai: {
    mobile: {
      common: {
        back: "Rudi",
        cancel: "Ghairi",
        close: "Funga",
        loading: "Inapakia…",
        notConnected: "Haijaunganishwa",
        open: "Fungua",
        refresh: "Onyesha upya",
        requestFailed: "Ombi limeshindwa. Jaribu tena.",
        retry: "Jaribu tena",
        save: "Hifadhi",
        saved: "Imehifadhiwa",
        search: "Tafuta",
        send: "Tuma",
        ready: "Tayari",
      },

      status: {
        ready: "Tayari",
        limited: "Imepunguzwa",
        not_connected: "Haijaunganishwa",
        error: "Hitilafu",
      },

      messageStatus: {
        sending: "Inatuma",
        sent: "Imetumwa",
        error: "Hitilafu",
        awaiting_confirmation: "Inasubiri uthibitisho",
      },

      home: {
        title: "Sabi AI",
        subtitle: "Msaidizi wa AI · Tafsiri · AI ya sauti",
        heroEyebrow: "AI",
        heroTitle: "Msaidizi wa AI",
        heroDescription:
          "Gumzo la AI · AI ya sauti · Tafsiri · Kumbukumbu · Premium AI.",
        quickAccess: "Ufikiaji wa haraka",
        capabilities: "Uwezo",
        safeActions: "Usalama",
        safeActionsDescription:
          "Usalama",
        safeActionsText:
          "Usalama",
        connection: "Muunganisho",
        userId: "ID ya mtumiaji",
        cleanWorkspaceTitle: "Msaidizi wa AI",
        cleanWorkspaceText:
          "Uwezo",
      },

      action: {
        chat: {
          title: "Gumzo la AI",
          description: "Gumzo la AI",
        },
        voice: {
          title: "AI ya sauti",
          description: "AI ya sauti",
        },
        translation: {
          title: "Tafsiri",
          description: "Tafsiri",
        },
        history: {
          title: "Historia",
          description: "Historia",
        },
        tasks: {
          title: "Kazi",
          description: "Kazi",
        },
        memory: {
          title: "Kumbukumbu",
          description: "Kumbukumbu",
        },
        setting: {
          title: "Mipangilio",
          description: "Mipangilio",
        },
        settings: {
          title: "Mipangilio",
          description: "Mipangilio",
        },
        premium: {
          title: "Premium AI",
          description: "Premium AI",
        },
        profileAi: {
          title: "AI ya wasifu",
          description: "AI ya wasifu",
        },
      },

      chat: {
        title: "Msaidizi wa AI",
        subtitle: "Gumzo la AI",
        roleUser: "ID ya mtumiaji",
        runtimeTitle: "Muunganisho",
        backendRequired: "Haijaunganishwa",
        emptyTitle: "Sabi AI",
        emptyText: "Gumzo la AI",
        cleanEmptyText:
          "Gumzo la AI",
        placeholder: "Gumzo la AI",
        emptyBackendResponse: "Bado tupu",
        errorMessage: "Hitilafu",
        thinking: "Inapakia…",

        assistantCenterTitle: "Msaidizi wa AI",
        assistantCenterDescription:
          "Uwezo",
        modeTitle: "Msaidizi wa AI",
        modeSubtitle: "Msaidizi wa AI",
        assistantMenuTitle: "Msaidizi wa AI",
        assistantMenuSubtitle:
          "Msaidizi wa AI",

        mode: {
          chatgpt: {
            title: "Msaidizi wa AI",
            short: "Msaidizi wa AI",
            description: "Msaidizi wa AI",
            prompt: "Msaidizi wa AI",
          },
          business: {
            title: "Msaidizi wa AI",
            short: "Msaidizi wa AI",
            description:
              "Msaidizi wa AI",
            prompt: "Msaidizi wa AI",
          },
          student: {
            title: "Msaidizi wa AI",
            short: "Msaidizi wa AI",
            description:
              "Msaidizi wa AI",
            prompt: "Msaidizi wa AI",
          },
          applicant: {
            title: "Msaidizi wa AI",
            short: "Msaidizi wa AI",
            description:
              "Msaidizi wa AI",
            prompt: "Msaidizi wa AI",
          },
          teacher: {
            title: "Msaidizi wa AI",
            short: "Msaidizi wa AI",
            description:
              "Msaidizi wa AI",
            prompt: "Msaidizi wa AI",
          },
        },

        toolsTitle: "Msaidizi wa AI",
        toolsSubtitle:
          "Msaidizi wa AI",
        toolsSheetTitle: "Msaidizi wa AI",
        toolsSheetSubtitle:
          "Msaidizi wa AI",

        camera: "Kamera",
        cameraDescription: "Kamera",
        uploadPhoto: "Picha",
        uploadPhotoDescription: "Picha",
        uploadVideo: "Video",
        uploadVideoDescription: "Video",
        uploadDocument: "Hati",
        uploadDocumentDescription: "Hati",
        webSearch: "Utafutaji wa wavuti",
        webSearchShort: "Utafutaji wa wavuti",
        webSearchDescription:
          "Utafutaji wa wavuti",

        voiceRecording: "AI ya sauti",
        permissionCamera: "Kamera",
        permissionPhoto: "Picha",
        permissionVideo: "Video",

        attachmentsSent: "Msaidizi wa AI",
        attachmentPhoto: "Picha",
        attachmentVideo: "Video",
        attachmentDocument: "Hati",
        attachmentAudio: "AI ya sauti",
        uploadContractNotice:
          "Msaidizi wa AI",

        safetyConfirmTitle: "Msaidizi wa AI",
        safetyConfirmMessage: "Msaidizi wa AI",
        safetyConfirmCancel: "Msaidizi wa AI",
        safetyConfirmContinue: "Msaidizi wa AI",
        securityNotice:
          "Msaidizi wa AI",

        prompt: {
          business: "Msaidizi wa AI",
          study: "Msaidizi wa AI",
          search: "Msaidizi wa AI",
          file: "Msaidizi wa AI",
        },
      },

      translation: {
        title: "Tafsiri",
        subtitle: "Tafsiri",
        heroEyebrow: "Tafsiri",
        realtimeTitle: "Tafsiri",
        premium:
          "Premium AI",
        source: "Maandishi chanzo",
        sourcePlaceholder: "Maandishi chanzo",
        textPhotoCamera: "Maandishi chanzo · Kamera · Picha",
        target: "Lugha lengwa",
        targetPlaceholder: "Lugha lengwa",
        translate: "Tafsiri",
        result: "Matokeo",
        inputPlaceholder: "Maandishi chanzo",
        outputPlaceholder: "Matokeo",
        camera: "Kamera",
        photo: "Picha",
        from: "Maandishi chanzo",
        to: "Lugha lengwa",
        detect: "Tafuta",
        selectLanguage: "Lugha lengwa",
        searchLanguage: "Tafuta",
        permissionCamera: "Kamera",
        permissionPhoto: "Picha",
        emptyText: "Bado tupu",
        noOutput: "Bado tupu",
        sameLanguage: "Lugha lengwa",
        photoReady: "Picha",
        cameraReady: "Kamera",
        detectCannotSwap: "Lugha lengwa",
        noImageOutput: "Bado tupu",
        detectedText: "Maandishi chanzo",
        messengerPathTitle: "Messenger",
        messengerReady:
          "Tayari",
        imageProcessing: "Inapakia…",
        textProcessing: "Inapakia…",
        metaDetected: "Tayari",
      },

      voice: {
        title: "AI ya sauti",
        subtitle: "AI ya sauti",
        nativeBridge: "Muunganisho",
        sttTtsBridgeTitle: "AI ya sauti",
        realBridgeDescription:
          "Daraja halisi la mobile: kurekodi kipaza sauti, matukio ya sauti ya native, kukabidhi transcript na kucheza audio URL. Text-only TTS haifanywi kwa uongo.",
        bind: "Muunganisho",
        start: "Anza",
        stop: "Simamisha",
        invoke: "Tuma",
        interrupt: "Simamisha",
        startRecording: "Rekodi",
        stopRecording: "Simamisha",
        recordingNow: "Rekodi",
        tapToSpeak: "AI ya sauti",
        session: "Muunganisho",
        recording: "Rekodi",
        audioSize: "AI ya sauti",
        audioDuration: "AI ya sauti",
        audioCapturedNotice:
          "Tayari",
        transcriptPlaceholder: "AI ya sauti",
        ttsTitle: "Cheza",
        ttsPlaceholder: "AI ya sauti",
        playLastTts: "Cheza",
        noFakeTts:
          "Hakuna sauti bandia: audio playback hufanya kazi tu backend/native TTS ikirudisha audio URL. Text-only TTS huonyeshwa kama maandishi hadi expo-speech au native TTS module iunganishwe.",
        runtimeLog: "Historia",
        noEvents: "Bado tupu",
        sessionStopped: "Simamisha",
        quickInvokeSent: "Imetumwa",
        transcriptReady: "Tayari",
        state: {
          idle: "Tayari",
          processing: "Inapakia…",
          requesting_permission: "Usalama",
          recording: "Rekodi",
          error: "Hitilafu",
          playing: "Cheza",
        },
        log: {
          info: "Tayari",
          bridge_bound: "Tayari",
          recording_started: "Rekodi",
          audio_captured: "Tayari",
          transcript_ready: "Tayari",
          tts_requested: "AI ya sauti",
          playback_started: "Cheza",
          interrupted: "Simamisha",
          error: "Hitilafu",
        },
      },

      settings: {
        title: "Mipangilio",
        heroEyebrow: "Mipangilio",
        heroTitle: "Mipangilio",
        heroDescription:
          "Mipangilio",
        subtitle: "Mipangilio",
        metricApi: "API",
        metricSource: "Maandishi chanzo",
        sourceLocalContract: "Mtoa huduma",
        connectionDetails: "Muunganisho",
        sections: "Uwezo",
        safetyTitle: "Usalama",
        safetyDescription:
          "Usalama",
        providerTitle: "Mtoa huduma",
        providerDescription:
          "Mtoa huduma",
        controlTitle: "Mipangilio",
        profileOwnerTitle: "AI ya wasifu",
        profileOwnerDescription:
          "AI ya wasifu",
        note: "Mipangilio",
      },

      memory: {
        title: "Kumbukumbu",
        subtitle: "Kumbukumbu",
        privacyMode: "Hali ya faragha",
        strict: "Kali",
        balanced: "Sawa",
        adaptive: "Inayobadilika",
        addInstruction: "Tuma",
        instructions: "Kumbukumbu",
        instructionPlaceholder: "Kumbukumbu",
        noAccusation:
          "Usalama",
        modeSaved: {
          strict: "Imehifadhiwa",
          balanced: "Imehifadhiwa",
          adaptive: "Imehifadhiwa",
        },
        instructionSaved:
          "Imehifadhiwa",
      },

      premium: {
        title: "Premium AI",
        heroEyebrow: "Premium AI",
        subtitle: "Premium AI",
        accessTitle: "Premium AI",
        noFakePayment:
          "Malipo hayafanywi kwa uongo hapa. Uwezeshaji lazima utoke kwenye mtiririko halisi wa uthibitisho wa Premium/COIN.",
        realtimeCallTranslationTitle: "Tafsiri",
        realtimeTranslationTitle: "Tafsiri",
        realtimeTranslationDescription:
          "Tafsiri",
        mediaTranslationTitle: "Tafsiri",
        mediaTranslationDescription:
          "Tafsiri",
        advancedAssistantActionsTitle: "Msaidizi wa AI",
        safeActionsTitle: "Usalama",
        safeActionsDescription:
          "Usalama",
        openProfilePremium: "Fungua",
        brandText: "Premium AI",
        badge: "Premium AI",
        safeBadge: "Usalama",
      },

      activity: {
        historyTitle: "Historia",
        historySubtitle: "Historia",
        tasksTitle: "Kazi",
        tasksSubtitle: "Kazi",
        emptyTitle: "Bado tupu",
        emptyText:
          "Bado tupu",
      },

      error: {
        ai_mobile_api_base_url_missing: "Hitilafu",
        ai_mobile_auth_required:
          "Hitilafu",
        ai_mobile_request_failed: "Hitilafu",
        ai_mobile_network_error:
          "Hitilafu",
        ai_voice_microphone_permission_denied:
          "Hitilafu",
        ai_voice_recording_already_active:
          "Hitilafu",
        ai_voice_recording_not_active: "Hitilafu",
        ai_voice_playback_command_missing: "Hitilafu",
        ai_voice_empty_tts_text: "Hitilafu",
        ai_voice_audio_url_missing:
          "Hitilafu",
      },
    },
  },
} as const;

export default SW_AI_MOBILE_LOCALE;