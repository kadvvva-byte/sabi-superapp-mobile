...
import { syncSabiProfileGroupToPublicDirectory, syncSabiProfileGroupsCollectionToPublicDirectory } from "../../src/modules/messenger/public-directory/profileDirectoryBridge";
import {
...
} from "../../src/modules/messenger/groups/groupPublicProfileRuntime";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { uploadUserPublicProfileMediaFile } from "../../src/shared/api/user-profile-api";
...
function buildMessengerRoomSubtitle(state: GroupProfileState) {
  ...
  if (description) return `${membersLabel} · ${description}`;
  return membersLabel;
}
      ...
      setForm((prev) => {
        ...
        if (mode === "photo") {
          ...
          next = pushHistory(next, {
            kind: "publication",
            title: tt("profile.photosScreen.header.title", "Rasmlar"),
            subtitle: tt("common.add", "Qo‘shish"),
          });
        }
...
groupAccess: "member",
  ...
  const upsertMember = useCallback(
    (member: GroupMember, origin: "manual" | "contact") => {
      ...
      const role = member.role.trim() || tt("common.member", "A’zo");
      ...
      setForm((prev) => {
        const next = pushHistory(
          ...
          },
          {
            ...
            title:
              origin === "contact"
                ? tt("messenger.addMembers", "Kontakt qo‘shish")
                : tt("messenger.addMember", "A’zo qo‘shish"),
            subtitle: `${name} · ${role}`,
          },
        );
  ...
  const handleApproveJoinRequest = useCallback(
    (requestUserId: string) => {
      ...
      setForm((prev) => {
        ...
        const role = tt("common.member", "A’zo");
  ...
  useEffect(() => {
    ...
    Alert.alert(
      tt("messenger.addMembers", "Kontakt qo‘shish"),
      `${selectedRouteMember.name} ${tt("common.add", "Qo‘shish")}`,
    );
  ...
  const handleAddAdmin = useCallback(() => {
    ...
    if (!id || !name) {
      Alert.alert(
        ...
        tt(
          ...
          "Admin qo‘shish uchun foydalanuvchi ID va ismni kiriting.",
        ),
      );
      return;
    }
    ...
    setForm((prev) => {
      ...
      const historyState = pushHistory(next, {
        kind: "community",
        title: tt("profile.groupScreen.admins.actions.add", "Admin qo‘shish"),
        subtitle: `${name} · ${role}`,
      });
      persistedState = historyState;
      return historyState;
      ...
      if (id === form.ownerUserId.trim()) {
        Alert.alert(
          ...
          tt(
            ...
            "Egasi admin ro‘yxatidan olib tashlanmaydi.",
          ),
        );
        return;
      }
  ...
  const handleAddBlacklist = useCallback(() => {
    ...
    if (!id || !name) {
      Alert.alert(
        ...
        tt(
          ...
          "Qora ro‘yxatga qo‘shish uchun foydalanuvchi ID va ismni kiriting.",
        ),
      );
      return;
    }
    ...
    if (id === form.ownerUserId.trim()) {
      Alert.alert(
        tt("profile.groupScreen.alerts.validation.ownerProtectedTitle", "Egasi himoyalangan"),
        tt("profile.groupScreen.alerts.validation.ownerBlacklistMessage", "Egasi qora ro‘yxatga qo‘shilmaydi."),
      );
      return;
    }
  ...
  const tabs = useMemo(
    () => [
      ...
      { key: "members" as TabKey, label: "A’zolar" },
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerBadge}>
            <Text style={styles.headerBadgeText}>{form.created ? `Faol · ${groups.length}` : `Yaratish · ${groups.length}`}</Text>
          </View>
        </View>
        ...
        >
          <View style={styles.groupShelfCard}>
            <View style={styles.groupShelfHeader}>
              <View style={styles.groupShelfTitleWrap}>
                ...
                <Text style={styles.groupShelfSubtitle}>{activeStoredGroup ? `${groups.length} ta guruh · ${activeStoredGroup.groupName}` : `${groups.length} ta guruh · bir nechta guruh saqlanadi`}</Text>
              </View>
          ...
          <View style={styles.heroCard}>
            ...
            <View style={styles.heroStatsRow}>
              <HeroChip width={listCardWidth} label="A’zolar" value={String(totalParticipants)} tone="blue" />
            ...
            <>
              ...
              <GlassSection title="Rasmlar" subtitle="Profil rasmlarini yuklash va boshqarish.">
                <MediaStrip
                  ...
                  type="photo"
                  emptyTitle="Hali rasmlar yo‘q"
                  emptyDescription="Birinchi rasmni qo‘shing."
          ...
          {activeTab === "members" ? (
            <>
              <GlassSection title="A’zolar" subtitle="A’zolar, adminlar va qora ro‘yxat boshqaruvi.">
                <View style={styles.gridWrap}>
                  <StatCard width={listCardWidth} label="A’zolar" value={String(totalParticipants)} accent={["#72E4CB", "#689AFF"]} />
                  ...
                  <StatCard width={listCardWidth} label="Qora ro‘yxat" value={String(form.blacklist.length)} accent={["#FFA08D", "#FF6D6D"]} />
                </View>
              </GlassSection>

              <GlassSection title="A’zo qo‘shish" subtitle="Kontaktlardan yoki qo'lda yangi ishtirokchini qo'shing.">
                <View style={styles.gridWrap}>
                  <InlineActionButton
                    width={listCardWidth}
                ...
                <View style={styles.membersHintCard}>
                  <Text style={styles.membersHintTitle}>Kontakt qo‘shish oqimi</Text>
                ...
                <StackList
                  items={form.members.map((item) => ({
                    ...
                    subtitle: `${item.id} · ${item.role || "A’zo"}`,
                  ...
                  emptyDescription="Birinchi ishtirokchini qo‘shing."
                />
              </GlassSection>
                ...
                <StackList
                  items={syncedForm.admins.map((item) => {
                    ...
                    return {
                      id: item.id,
                      title: item.name || item.id,
                      subtitle: `${item.id} · ${item.role || "Admin"}`,
                  ...
                  emptyTitle="Hozircha adminlar yo‘q"
                  emptyDescription="Adminlarni shu yerda boshqaring."
                />
              </GlassSection>

              <GlassSection title="Qora ro‘yxat" subtitle="Cheklangan foydalanuvchilar ro'yxati.">
                <View style={styles.formGrid}>
                  <CompactInput
                    label="Foydalanuvchi ID"
                ...
                <CompactInput
                  ...
                  placeholder="Spam, suiste’mol, firibgarlik va hokazo"
                />
                <Pressable style={styles.softButton} onPress={handleAddBlacklist}>
                  <Ban size={16} color={RED} />
                  <Text style={styles.softButtonText}>Qora ro‘yxatga qo‘shish</Text>
                </Pressable>
                ...
                <StackList
                  items={form.blacklist.map((item) => ({
                    id: item.id,
                    title: item.name || item.id,
                    subtitle: `${item.id} · ${item.reason || "Cheklangan"}`,
                    icon: <Ban size={16} color={RED} />,
                    actionText: "Olib tashlash",
                    onAction: () => handleRemoveBlacklist(item.id),
                    tone: "danger" as const,
                  }))}
                  emptyTitle="Qora ro‘yxat bo‘sh"
                  emptyDescription="Cheklangan foydalanuvchilar hozircha yo‘q."
                />
              </GlassSection>
            </>
          ) : null}
          ...
          {activeTab === "control" ? (
            <>
              <GlassSection title="Egasi ma’lumotlari" subtitle="Egasi ID bo‘sh bo‘lsa, yaratish vaqtida avtomatik yaratiladi.">
                <CompactInput
                  label="Egasi ismi"
              ...
              <GlassSection title="Nashr va ommaviy qatlam" subtitle="Ommaviy va yopiq ko'rinish qoidalari.">
                ...
                <ToggleRow
                  ...
                  subtitle="Guruh bo‘limini profil ichida ko‘rsatish."
                ...
                <ToggleRow
                  ...
                  subtitle="Havola va profil ko‘rib chiqish kartalarini ko‘rsatish."
                ...
                <CompactInput
                  ...
                  placeholder="Ommaviy ko‘rinish sarlavhasi"
                />
                <CompactInput
                ...
                <CompactInput
                  ...
                  placeholder="Ommaviy ko‘rinish uchun qisqa xulosa"
                  multiline
                />
              </GlassSection>

              <GlassSection title="Kirish va moderatsiya" subtitle="A’zolar, yozish va moderatsiya qoidalari.">
                <ToggleRow
              ...
              <GlassSection title="Tarix" subtitle="Oxirgi o'zgarishlar.">
                {form.history.length ? (
                  ...
                  <EmptyState
                    ...
                    description="Harakatlar paydo bo‘lganda bu yerda tarix yig‘iladi."
                  />
                )}
              </GlassSection>
            </>
