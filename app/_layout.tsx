...
function normalizeIncomingCallKind(payload: IncomingMessengerCallPayload) {
  const raw = String(payload.kind ?? payload.type ?? "").trim().toLowerCase();
  ...
  useEffect(() => {
    ...
    const handleIncomingCall = (payload: unknown) => {
      ...
      if (
        ...
        (lastIncomingCallKeyRef.current?.startsWith(callKey) ||
          lastIncomingCallKeyRef.current?.includes(callKey)) &&
      ...
      if (isPlainDirectForSameActiveCall) {
        console.log(
          "[sabi-call:incoming-route] BLOCK direct route after handoff",
          "callId=" + callKey,
          "last=" + lastIncomingKeyText,
        );
        return;
      }
      ...
      pendingIncomingCallKeyRef.current = nextKey;
      pendingIncomingCallTimerRef.current = setTimeout(() => {
        if (pendingIncomingCallKeyRef.current !== nextKey) return;
        pendingIncomingCallTimerRef.current = null;
        pendingIncomingCallKeyRef.current = null;
        pushIncomingRoute();
      }, 350);
